...
- 되면 좋은:
  - **대시보드 — 정적 HTML 리포트**. 명령 한 번으로 `index.html`을 생성해 브라우저로 연다 (서버·빌드 없음). `aaa v1(A)` / `aaa v2(B)`를 한 화면에서 확인하는 용도
...
**Phase 2 — 버전 등록 스킬 + 이력 SSOT + `version-history-guard` hook** (2.5시간)
- 완료: 샘플 `aaa`에 v1(작업자 A) → v2(작업자 B) 순으로 등록했을 때 `npm test`가 아래 셋을 전부 통과한다
  1. **v1 파일이 그대로 존재한다** (덮어쓰지 않음)
  2. 이력에 **2건**이 있고 작업자가 각각 `A`, `B`로 기록돼 있다
...
## ⑤ 완료를 판정할 방법 (검증 게이트)
- 기계가 판정하는 것:
  - **`npm test`** (`node --test`) — ④ Phase 0~3의 완료 조건 전부 (샘플 2/2, v1 잔존, 이력 2건과 작업자 A·B, 브리핑 네 항목 2/2)
...
- **대시보드**
  ...
  - 「되면 좋은」에 두는 것이 맞나, v1·v2 확인 요구를 생각하면 「반드시」여야 하나
