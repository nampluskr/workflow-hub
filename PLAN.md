...
- 되면 좋은:
  - **개발 코드·매뉴얼에 대한 LLM wiki 구성** (REQUEST.md 최종 목표 중 하나). **(2026-08-13 진행 중, 같은 날 `Phase 4`로 정식 번호 부여됨 — 아래 ④ Phase 4 참고)** 착수 방향 확정: 프로젝트별 대화형 Q&A 서브에이전트, `NAVIGATION.md`(Phase 3 재정의 참고)만 읽고 코드/보고서 전체는 읽지 않는 구조. `NAVIGATION.md` 생성까지가 Phase 3 범위였고, Q&A 서브에이전트 자체 구현이 Phase 4다.
...
**Phase 4 — LLM wiki: 프로젝트별 대화형 Q&A 서브에이전트 (2026-08-13, 정식 번호 부여, 착수 전)**

③의 "되면 좋은" 항목이던 LLM wiki를 정식 Phase로 승격한다. 「반드시」로 다시 승격하는 것은 아니다 — 범위는 그대로 선택 사항이지만, 번호를 붙여 다른 Phase와 같은 방식(완료 기준·백로그 항목)으로 추적한다.

- 구조: 샘플별 대화형 Q&A 서브에이전트. 질문을 받으면 **`NAVIGATION.md`부터** 읽고, 거기서 가리키는 report.md 절·`src/main.py:<줄>`로만 점프해 답한다 — 특정 버전의 report.md 전체나 코드 전체를 처음부터 다시 읽지 않는다(Phase 3 재정의에서 확정한 전제).
- 완료 기준(초안, 착수 시 재확인 필요 — 아직 사람 승인 전):
  1. 두 샘플(`display-defect-rate`, `display-image-inspection`) 각각에 최소 2건의 질문에 답할 수 있다
  2. 각 답변이 `NAVIGATION.md`의 실제 행(기능/주제)을 인용하고, 인용한 report.md 절 제목·코드 위치가 실재한다 — 사람이 눈으로 확인(⑤의 "사람이 눈으로 보는 것"에 해당, 기계 판정 불가 영역)
  3. `NAVIGATION.md`에 없는 내용을 질문했을 때 지어내지 않고 "네비게이션 인덱스에 없다"고 답한다
- 아직 정하지 않은 것: 서브에이전트 정의 파일 위치·이름, 호출 방식(사용자가 직접 이름 호출? 스킬 경유?), `npm test`로 옮길 수 있는 기계 판정 부분이 있는지.
