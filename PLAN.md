...
- 되면 좋은:
  - **개발 코드·매뉴얼에 대한 LLM wiki 구성** (REQUEST.md 최종 목표 중 하나). **(2026-08-13 진행 중)** 착수 방향 확정: 프로젝트별 대화형 Q&A 서브에이전트, `NAVIGATION.md`(Phase 3 재정의 참고)만 읽고 코드/보고서 전체는 읽지 않는 구조. `NAVIGATION.md` 생성까지가 이번 세션 범위이고, Q&A 서브에이전트 자체 구현은 다음 단계.
