...
def overall_defect_rate(rows):
    produced = sum(row["produced_qty"] for row in rows)
    defect = sum(row["defect_qty"] for row in rows)
    return defect / produced if produced else 0.0
...
def main(argv):
    ...
    print(f"\n전체 평균 불량율: {overall_defect_rate(rows):.2%}")
