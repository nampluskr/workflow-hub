...
- 출력: 콘솔에 설비별/모델별 불량율 표를 출력하고, `data/report.csv`에 같은 내용을 `scope, id, produced_qty, defect_qty, defect_rate` 열로 저장한다.
...
## 수정 시 참고
- `main.py` — 전체 로직. `load_rows`(CSV 읽기) → `aggregate`(설비/모델별 합산 및 불량율 계산) → `print_table`/`write_report`(출력) 순서로 구성.
