...
2. **`.claude/version-history/` 아래 파일을 `Edit`나 `Write`로 직접 건드리지 않는다** — `version-history-guard` hook이 막는다. 반드시 아래 CLI를 Bash로 실행한다:
...
- 이 스킬은 **등록만** 한다. 이력 조회는 별도 스킬 없이 `history.json`을 직접 읽으면 된다(인수인계 브리핑 서브에이전트가 이 방식을 쓴다).
