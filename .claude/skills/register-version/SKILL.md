...
2. **`tools/version.mjs register`를 부르기 전에, `wiki-writer` 서브에이전트를 먼저 호출해 `samples/<프로젝트명>/wiki/`를 최신화한다** (CLAUDE.md §6). 이 순서가 바뀌면 안 된다 — 등록 스크립트가 `samples/<프로젝트명>/`를 그대로 스냅샷에 복사하므로, `wiki/`가 그 시점에 이미 있어야 이번 버전 스냅샷에 포함된다.
...
- 이 스킬은 **등록만** 한다(2단계의 `wiki-writer` 호출 포함). 이력 조회는 별도 스킬 없이 `history.json`을 직접 읽으면 된다.
